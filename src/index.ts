...
export class SABnzbd {
   ...
   * ```
   */
  constructor(private readonly host: string, private readonly port: string, private readonly apiKey: string) {
   ...
   */
  public async addURL(url: string, params: AddURLParams = {}): Promise<string[]> {
    ...
    const { nzo_ids } = await this.request(searchParams) as { status: boolean, nzo_ids: string[] };
    return nzo_ids;
  }
   ...
   * @param params Input parameter
   */
  public async addFileAndWaitTillFinish(url: string, params: AddURLParams = {}): Promise<string[]> {
  ...
  public async history(ids: string[] = []): Promise<History> {
    ...
    const { history } = await this.request(searchParams) as { history: History };
    return history;
  }
